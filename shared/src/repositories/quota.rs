...
impl QuotaManager {
    fn plan_limits() -> HashMap<bool, (u32, u32)> {
        HashMap::from([
            (false, (5, 200)), // (secret_quota, request_quota)
            (true, (50, 5_000)),
        ])
    }

    pub fn get_quotas(user: &UserDocument) -> (u32, u32) {
        let plan = if user.has_paid { true } else { false };
    ...
    pub async fn enforce_request_quota(
        ...
        let user_doc = user_repo.get_user_by_id(&user.id.to_string()).await?;
        if let Some(mut user_doc) = user_doc {
            ...
            Err(anyhow!("User not found while enforcing request quota."))
        }
    }
    ...
    pub async fn update_quota_on_secret_create(
        ...
        if let Some(mut user_doc) = user_doc {
            ...
            Err(anyhow!("User not found when updating secret quota."))
        }
    }
    ...
    pub async fn update_quota_on_secret_delete(
        ...
        if let Some(mut user_doc) = user_doc {
            ...
            // Cap the quota to the plan's max
